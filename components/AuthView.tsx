...
const AuthView: React.FC<AuthViewProps> = ({ onAuthSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulazione autenticazione cloud
    setTimeout(() => {
      const mockUser: User = {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 md:p-12 font-inter">
      <div className="w-full max-w-xl animate-fadeIn">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.3em] mt-2">Sincronizzazione Universitaria</p>
        </div>
        ...
        <div className="bg-white rounded-[3.5rem] shadow-2xl shadow-blue-500/10 p-10 md:p-16 border border-slate-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              ) : isLogin ? 'Entra nel PKM' : 'Crea Account Cloud'}
            </button>
          </form>

          <p className="mt-8 text-center text-slate-400 text-xs font-medium">
            I tuoi dati sono crittografati e accessibili solo da te.
          </p>
        </div>
      </div>
    </div>
